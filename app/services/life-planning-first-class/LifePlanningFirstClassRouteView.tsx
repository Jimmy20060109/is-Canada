    ...
    <main className="bg-[#f3f4f6] text-zinc-900">
      <section className="mx-auto max-w-7xl px-6 pb-10 pt-16 md:pt-20">
        <div className="rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm md:p-12">
          <p className="text-sm font-semibold uppercase tracking-[0.15em] text-[#7f2542]">Life Planning</p>
          <h1 className="mt-4 text-4xl font-black tracking-tight text-[#12285f] md:text-6xl">3.3 人生规划第一课</h1>
          <p className="mt-6 max-w-4xl text-xl font-bold leading-relaxed text-zinc-900 md:text-3xl">
            以生涯规划开启天赋和热爱，
            <span className="block text-[#12285f]">让孩子活出闪闪发光的人生。</span>
          </p>
        </div>
      </section>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {painPoints.map((item, idx) => (
            <article key={item} className="rounded-2xl border border-zinc-200 bg-white p-6">
              <p className="text-sm font-bold tracking-wide text-[#12285f]">Pain 0{idx + 1}</p>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-10">
        <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm md:p-8">
          <h2 className="text-2xl font-extrabold text-[#12285f] md:text-3xl">我们能解决的问题</h2>
          ...
          {misconceptions.map((item, idx) => (
            <div key={item} className="rounded-2xl border border-zinc-200 bg-white px-5 py-4">
              <p className="text-sm text-zinc-700 md:text-base">
                <span className="mr-2 font-bold text-[#12285f]">误区 {idx + 1}</span>
                {item}
              </p>
            </div>
          ))}
      ...
      <section className="mx-auto max-w-7xl px-6 pb-16 md:pb-24">
        ...
        <div className="mt-8 rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm md:p-8">
          <p className="text-lg font-bold leading-relaxed text-zinc-900 md:text-2xl">
            “父母唯一应该为孩子做的，就是找到孩子的激情所在，
            鼓励 TA 全力以赴去追求，发挥得淋漓尽致。”
          </p>
          <p className="mt-3 text-sm text-zinc-600">—— 巴菲特</p>
