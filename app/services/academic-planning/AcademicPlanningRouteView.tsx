    ...
    <main className="bg-[#f3f4f6] text-zinc-900">
      <section className="mx-auto max-w-7xl px-6 pb-10 pt-16 md:pt-20">
        <div className="rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm md:p-12">
          <p className="text-sm font-semibold uppercase tracking-[0.15em] text-[#7f2542]">Academic Planning</p>
          <h1 className="mt-4 text-4xl font-black tracking-tight text-[#12285f] md:text-6xl">3.2 升学规划</h1>
          <p className="mt-6 max-w-4xl text-xl font-bold leading-relaxed text-zinc-900 md:text-3xl">
            真正好的留学规划，不只获得名校“录取通知书”，
            <span className="block text-[#12285f]">更要找到“人生使用说明书”。</span>
          </p>
        </div>
      </section>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {painPoints.map((item, idx) => (
            <article key={item} className="rounded-2xl border border-zinc-200 bg-white p-6">
              <p className="text-sm font-bold tracking-wide text-[#12285f]">0{idx + 1}</p>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-10">
        <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm md:p-8">
          <h2 className="text-2xl font-extrabold text-[#12285f] md:text-3xl">幸福树模型 · 双主干</h2>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <article className="rounded-2xl border border-zinc-200 bg-[#fafbff] p-5">
              <h3 className="text-lg font-bold text-[#12285f]">学术主干</h3>
            ...
            <article className="rounded-2xl border border-zinc-200 bg-[#fafbff] p-5">
              <h3 className="text-lg font-bold text-[#12285f]">履历主干</h3>
              <ul className="mt-3 space-y-2 text-sm text-zinc-700">
          ...
          <ul className="space-y-3 text-sm leading-relaxed text-zinc-700 md:text-base">
            {ossdPoints.map((point) => (
              <li key={point} className="flex items-start gap-3">
                <span className="mt-1 inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#12285f] text-xs font-bold text-white">
                  ✓
                </span>
                <span>{point}</span>
              </li>
